    ...
    <div className='space-y-4 border-t pt-6'>
      ...
      <div className='border-border rounded-lg border'>
        <button
          ...
          aria-expanded={open}
          onClick={() => setOpen(v => !v)}
          className='hover:bg-muted/40 flex w-full items-start justify-between gap-3 px-4 py-3 text-left'
        >
          <div className='min-w-0 flex-1'>
            ...
            {!open && goodsLineCount > 0 ? (
              <p className='text-muted-foreground mt-1 truncate text-xs font-normal normal-case'>
                {goodsLineCount}{' '}
                {goodsLineCount === 1
                  ? tBookingForm('goodsItemsSingular')
                  : tBookingForm('goodsItemsPlural')}{' '}
          ...
          <ChevronDown
            className={cn(
              'size-5 shrink-0 text-violet-600 transition-transform',
              open && 'rotate-180'
            )}
          />
        </button>
        ...
        >
          <div className='min-h-0 overflow-hidden' inert={!open}>
            <div className='border-border max-h-[min(40vh,18rem)] space-y-4 overflow-y-auto overscroll-contain border-t px-4 py-4'>
              <p className='text-muted-foreground text-sm'>
                {tBookingForm('goodsHint')}
              </p>
              ...
              {catalog.length > 0 && branchId ? (
                <div className='space-y-2'>
                  ...
                  <p className='text-muted-foreground text-xs'>
                    {tBookingForm('proShopCatalogHint')}
                  </p>
        ...
        <div className='space-y-2 text-sm'>
          <div className='flex justify-between gap-4'>
            <span className='text-muted-foreground'>
              {tBookingForm('courtRentalFee')}
            </span>
            <span className='font-medium tabular-nums'>
              {formatMoney(courtRental, locale)}
            </span>
          </div>
          <div className='flex justify-between gap-4'>
            <span className='text-muted-foreground'>
              {tBookingForm('goodsSubtotal')}
            </span>
            <span className='font-medium tabular-nums'>
              {formatMoney(goodsSubtotal, locale)}
            </span>
          </div>
          <div className='border-border flex justify-between gap-4 border-t pt-2 text-base'>
